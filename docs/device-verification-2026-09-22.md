...
| 7 | App 啟動（程序存活） | ✅ 通過 | am start Status=timeout；pid=3891　→ timeout 只是 -W 等待窗口唔夠長，程序實際已起 |
...
| 19 | WebView devtools socket 存在（debug build 可遠端除錯） | ✅ 通過 | webview_devtools_remote_3891 |
| 20 | 找到 WebView 頁面 target | ✅ 通過 | https://macau-pos-system.vercel.app/ |
| 21 | WebView 載入的是 Vercel POS | ✅ 通過 | https://macau-pos-system.vercel.app/ |
...
| 34 | 本機收到 ESC/POS 字節（出紙鏈路真的通） | ✅ 通過 | 756 bytes；開頭 24 bytes: 1b 40 1b 61 01 1d 21 00 1b 21 00 1c 21 00 1b 21 00 1b 45 00 1b 33 1e 1c |
| 35 | 含 ESC @ 初始化（1b 40） | ✅ 通過 | 出現 2 次 |
| 36 | 含 GS v 0 點陣圖（QR 走點陣而非 GS ( k） | ⚠️ 待確認 | 無 QR（測試單可能不含 QR，非錯誤） |
| 37 | 每行有 clearMagnify 歸零痕跡（GS!/ESC!/FS! 三套） | ✅ 通過 | 歸零指令出現 78 次（v1.1.4 修「一條線變兩條」的關鍵） |
...
| 39 | 原始字節已存檔供人手核對 | ✅ 通過 | C:\dev\macauPosAndroid\docs\device-print-capture-2026-09-22.bin |
| 40 | 標籤機（family=label, 100x75mm）有送出出紙字節 | ✅ 通過 | 378 bytes；testPrint 回應 (evaluate timeout) |
